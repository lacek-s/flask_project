...
@admin.route("/admin/new/", methods=["GET"])
def view_admin_new():
    if "logged" not in session:
        flash("You must me logged in.", "alert-danger")
...
def view_article_editor(art_id):
    ...
    return render_template("mod_admin/article_not_found.jinja", art_id=art_id)
...
def login_user():
    ...
    if login_form.validate():
        ...
        if user and user.check_password(login_form.password.data):
            ...
            return render_template("mod_admin/.jinja", form=login_form )
    else:
        for error in login_form.errors:
            flash("{} is missing.".format(error), "alert-danger")
        return redirect(url_for("admin.view_login"))
